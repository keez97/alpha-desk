...
function Tooltip({ text, children }: { text: string; children: React.ReactNode }) {
  return (
    <span className="relative group/tip cursor-help">
      {children}
      <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 w-max max-w-[180px] rounded bg-neutral-800 border border-neutral-700 px-2 py-1.5 text-xs leading-snug text-neutral-300 opacity-0 group-hover/tip:opacity-100 transition-opacity z-50 shadow-lg">
        {text}
      </span>
    </span>
  );
    ...
    <div className="flex items-center h-4 mb-1 border-b border-neutral-800/50 pb-1" style={{ gap: 8 }}>
      ...
      <div className="flex-1 text-center">
        <Tooltip text="Bar length shows the relative size of the net position. The larger group (hedger or spec) fills the full bar; the smaller scales proportionally. Green = net long, Red = net short.">
          <span className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Positioning</span>
        </Tooltip>
      </div>
      ...
      <div style={{ width: 60, flexShrink: 0 }} className="text-right">
        <Tooltip text="Net contracts = long minus short contracts held by that trader group.">
          <span className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Net</span>
        </Tooltip>
      </div>
      ...
      <div style={{ width: 40, flexShrink: 0 }} className="text-center">
        <Tooltip text="Whether the group is net long (buying) or net short (selling) overall.">
          <span className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Side</span>
        </Tooltip>
      </div>
      ...
      <div style={{ width: 28, flexShrink: 0 }} className="text-right">
        <Tooltip text="Percentile rank vs 52-week range. P90+ or P10- = extreme positioning (amber). P50 = middle of the range.">
          <span className="text-xs font-medium text-neutral-500 uppercase tracking-wider">%ile</span>
        </Tooltip>
      </div>
      ...
      <div style={{ width: 16, flexShrink: 0 }} className="text-center">
        <Tooltip text="Week-over-week change in net positioning. ↑ = added to position, ↓ = reduced position.">
          <span className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Wk</span>
        </Tooltip>
      </div>
    </div>
  );
...
function PositioningBar({
  ...
  return (
    <div className="flex items-center h-5" style={{ gap: 8 }}>
      {/* Label */}
      <span className="text-xs text-neutral-500" style={{ width: 48, flexShrink: 0 }}>{label}</span>
      ...
      {/* Net position */}
      <span className={`text-xs font-mono font-bold text-right ${textColor}`} style={{ width: 60, flexShrink: 0 }}>
        {isLong ? '▲ ' : '▼ '}{isLong ? '+' : ''}{formatContracts(net)}
      </span>

      {/* Direction badge */}
      <span className={`text-xs font-bold text-center ${textColor}`} style={{ width: 40, flexShrink: 0 }}>
        {isLong ? '▲' : '▼'} {direction}
      </span>
      ...
      {/* Weekly change */}
      <span className="text-xs text-center" style={{ width: 16, flexShrink: 0 }}>
  ...
  return (
    <div className="py-1.5 border-b border-neutral-800/30 last:border-0">
      ...
      <div className="flex items-center gap-2 mb-1">
        <span className="text-xs font-bold text-neutral-200">{market.name}</span>
        <span className="text-xs font-mono text-neutral-500">({market.ticker})</span>
        {market.divergence && (
          <Tooltip text="Hedgers and speculators are positioned on opposite sides with a wide gap — watch for reversal.">
            <span className="text-xs px-1.5 py-0.5 rounded-full bg-amber-900/30 text-amber-400 font-medium">DIV</span>
          </Tooltip>
        )}
      </div>
...
function AlertBadge({
  ...
  return (
    <div className={`rounded border p-1.5 text-xs ${colors}`}>
  ...
  return (
    <div ref={ref} className="absolute right-0 top-6 z-50 w-64 bg-neutral-900 border border-neutral-700 rounded-lg shadow-xl p-3 space-y-2">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-bold text-neutral-300">Customize Markets</span>
        <button onClick={onReset} className="text-xs text-blue-400 hover:text-blue-300 cursor-pointer">
          Reset defaults
        </button>
      </div>
      {Object.entries(grouped).map(([cat, markets]) => (
        <div key={cat}>
          <div className="text-xs font-bold text-neutral-500 uppercase mb-0.5">{cat}</div>
          <div className="space-y-0.5">
            {markets.sort((a, b) => a.sort_order - b.sort_order).map(m => (
              <label key={m.ticker} className="flex items-center gap-2 cursor-pointer py-0.5 hover:bg-neutral-800/50 rounded px-1">
                ...
                />
                <span className="text-xs text-neutral-300">{m.name}</span>
                <span className="text-xs text-neutral-500 font-mono ml-auto">{m.ticker}</span>
              </label>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-2 rounded border border-neutral-800 bg-neutral-900/50 p-3">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-neutral-400">COT Positioning</span>
      ...
      <div className="flex gap-4 text-xs text-neutral-500">
        ...
        <Tooltip text="Divergence: hedgers and speculators are on opposite sides with a wide percentile gap (>30pts). Smart money vs crowd.">
          <div className="flex items-center gap-1">
            <span className="text-xs px-1 rounded-full bg-amber-900/30 text-amber-400 font-medium">DIV</span>
      ...
      {/* Footer */}
      <div className="border-t border-neutral-800 pt-1.5 text-xs text-neutral-500">
